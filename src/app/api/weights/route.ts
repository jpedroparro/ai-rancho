...
export const POST = withAuth(async (req, _, userId) => {
  ...
  if (!data.animalId || !data.date || !data.weight) return NextResponse.json({ error: 'animalId, date e weight sao obrigatorios' }, { status: 400 })
  const farmIds = await createFarmService().resolveFarmIds(userId, data.farmId)
  const record = await createWeightService().create({ ...data, farmId: farmIds[0] ?? null })
